...
impl Display for ImageRendering {
  fn fmt(&self, f: &mut Formatter) -> fmt::Result {
    match self {
      ...
      Self::Pixelated => write!(f, "pixelated"),
    }
  }
}
...
impl Media {
  ...
  const TABLE: &'static [(&'static str, BrotliEncoderMode, Media, &'static [&'static str])] = &[
    ...
    ("image/png", BROTLI_MODE_GENERIC, Media::Image(ImageRendering::Pixelated), &["png"]),
    ("image/svg+xml", BROTLI_MODE_TEXT, Media::Iframe, &["svg"]),
    ("image/webp", BROTLI_MODE_GENERIC, Media::Image(ImageRendering::Auto), &["webp"]),
