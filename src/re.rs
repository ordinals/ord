...
lazy_static! {
  ...
  pub(crate) static ref COINKITE_SATSCARD_URL: Regex =
  ...
  pub(crate) static ref INSCRIPTION_NUMBER: Regex = re(r"-?[0-9]{1,63}");
  pub(crate) static ref ORDINALS_SATSCARD_URL: Regex =
    re(r"https://ordinals.com/satscard\?(?<query>.*)");
  pub(crate) static ref OUTPOINT: Regex = re(r"[[:xdigit:]]{64}:\d+");
  pub(crate) static ref RUNE_ID: Regex = re(r"[0-9]{1,63}:[0-9]+");
