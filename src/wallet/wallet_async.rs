...
impl WalletAsync {
  pub(crate) fn build(name: String, no_sync: bool, options: Options, rpc_url: Url) -> Result<Self> {
    let bitcoin_client = {
      let client = check_version(options.bitcoin_rpc_client(Some(name.clone()))?)?;
    ...
    tokio::runtime::Builder::new_multi_thread()
      ...
      .block_on(async move {
        ...
        for output in utxos.keys() {
          output_info.insert(
            output.clone(),
            Self::get_output(&ord_client, &output).await?,
          );
        }

        let inscriptions_ids = output_info
          .iter()
          .map(|(_output, info)| info.inscriptions.clone())
          .flatten()
  ...
  fn get_output_sat_ranges(&self) -> Result<Vec<(OutPoint, Vec<(u64, u64)>)>> {
    ensure!(
      self.has_sat_index,
  ...
  fn find_sat_in_outputs(&self, sat: Sat) -> Result<SatPoint> {
    ensure!(
      self.has_sat_index,
      "ord index must be built with `--index-sats` to use `--sat`"
    );
  ...
  fn get_inscriptions(&self) -> BTreeMap<SatPoint, Vec<InscriptionId>> {
  ...
  pub(crate) fn get_parent_info(
    ...
    if let Some(parent_id) = parent {
      let satpoint = self
        .inscription_info
        .get(&parent_id)
        .ok_or_else(|| anyhow!(format!("parent {parent_id} not in wallet")))?
        .satpoint;
  ...
  pub(crate) fn initialize_from_descriptors(&self, descriptors: Vec<Descriptor>) -> Result {
    ...
    let descriptors = Self::check_descriptors(&self.name, descriptors)?;
