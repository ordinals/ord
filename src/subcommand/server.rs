...
impl Server {
  ...
  async fn artwork(
    ...
    task::block_in_place(|| {
      ...
      let body = inscription.into_body().ok_or_not_found(|| {
        format!("inscription {inscription_id} content")
      })?;
  ...
  fn extract_audio_artwork_direct(audio_data: Vec<u8>) -> ServerResult {
    ...
    match symphonia::default::get_probe().format(&hint, media_source, &format_opts, &metadata_opts) {
      Ok(probed) => {
        ...
        Err(ServerError::NotFound("No artwork found in audio file metadata".into()))
      }
      Err(_) => {
        Err(ServerError::NotFound("Unable to parse audio file".into()))
      }
    }
  }
