...
impl Batch {
  pub(crate) fn inscribe(
    ...
    let commit_tx_change = [
      get_change_address(&client, &self.options)?,
      get_change_address(&client, &self.options)?,
    ];
    ...
    if !self.no_backup {
      Inscribe::backup_recovery_key(&client, recovery_key_pair, self.options.chain().network())?;
    }
...
impl Inscribe {
  pub(crate) fn run(self, options: Options) -> SubcommandResult {
    ...
    .inscribe(&index, &client, &utxos, parent_info)
  }
