...
Get hex-encoded block with height or block hash in `<QUERY>`. In the event of a
future change to Bitcoin that changes transaction serialization in a
...
```bash
curl -s http://0.0.0.0:80/r/block/0
```
