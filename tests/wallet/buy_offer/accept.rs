...
fn accepted_rune_offer_works() {
  ...
  let create = CommandBuilder::new(format!(
    "--regtest wallet buy-offer create --outgoing {}:{} --amount 1btc --fee-rate 0 --utxo {}",
    250,
    Rune(RUNE),
    outpoint
  ))
  ...
  CommandBuilder::new(format!(
    "--regtest wallet buy-offer accept --outgoing {}:{} --amount 1btc --psbt {} --dry-run",
    250,
    Rune(RUNE),
    create.psbt
  ))
  ...
  CommandBuilder::new(format!(
    "--regtest wallet buy-offer accept --outgoing {}:{} --amount 1btc --psbt {}",
    250,
    Rune(RUNE),
    create.psbt,
  ))
...
#[test]
fn outgoing_must_contain_an_inscription_if_inscription_offer() {
  let core = mockcore::spawn();
...
#[test]
fn outgoing_contains_unexpected_outgoing_inscription() {
  let core = mockcore::spawn();
...
fn must_index_runes_if_rune_offer() {
  ...
  error_case(
    ...
    &format!("1000:{}", Rune(RUNE)),
    "error: accepting rune offer with `buy-offer` requires index created with `--index-runes` flag\n",
  );
}
...
fn outgoing_contains_unexpected_rune_balance_in_rune_offer() {
  ...
  .expected_exit_code(1)
  .expected_stderr(format!(
    "error: unexpected outgoing input rune {} balance ({} vs. {})\n",
    rune, 500, 250
  ))
  .run_and_extract_stdout();
}
