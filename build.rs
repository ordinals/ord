use std::{process::Command, str, path::Path, fs};
...
fn ensure_opus_decoder() -> Result<(), Box<dyn std::error::Error>> {
  ...
  for (filename, expected_hash) in OPUS_DECODER_FILES {
    ...
    if hash != *expected_hash {
      return Err(format!(
        "Checksum mismatch for {filename}: expected {expected_hash}, got {hash}"
      ).into());
    }

    fs::write(&path, &bytes)?;
    println!("cargo:warning={filename} downloaded and verified ({} bytes)", bytes.len());
  }
